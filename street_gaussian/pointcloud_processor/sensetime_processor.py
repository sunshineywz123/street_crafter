...
from data_processor.sensetime_processor.sensetime_helpers import load_centered_vehicle_poses, load_calibration, load_track, \
    image_filename_to_cam, image_filename_to_frame, file_path_to_frame_name, image_heights, image_widths, load_sensor_sync, camera_names_dict
...
class SensetimePointCloudProcessor(BasePointCloudProcessor):
    def __init__(self):
        ...
        # load camera names
        self.camera_id_names_dict = dict()
        for camera_name in camera_names_dict:
            self.camera_id_names_dict[int(camera_names_dict[camera_name])] = camera_name

        # load camera poses
        self.ego_frame_poses = load_centered_vehicle_poses(self.datadir)
        ...
        # load sync file
        sensor_sync = load_sensor_sync(self.datadir)
        self.frame_name_list = list(sensor_sync.keys())
        num_frames = len(self.frame_name_list)
        # breakpoint()
    ...
    def read_lidar_ply(self):
        ...
        bkgd_ply_list = [x for x in bkgd_ply_list if self.frame_name_list.index(file_path_to_frame_name(os.path.basename(x))) >= self.start_frame and self.frame_name_list.index(file_path_to_frame_name(os.path.basename(x))) <= self.end_frame]

        for i, bkgd_ply_path in enumerate(tqdm(bkgd_ply_list, desc='Reading Background ply')):
            bkgd_ply = fetchPly(bkgd_ply_path)
            frame_name = file_path_to_frame_name(os.path.basename(bkgd_ply_path))

            mask = bkgd_ply.mask 
            xyz_vehicle = bkgd_ply.points[mask]
            xyz_vehicle_homo = np.concatenate([xyz_vehicle, np.ones_like(xyz_vehicle[..., :1])], axis=-1)
            xyz_world = xyz_vehicle_homo @ self.ego_frame_poses[frame_name].T
            ...
            for cam in self.cams:
                camera_name = self.camera_id_names_dict[cam]
                K = self.intrinsics[camera_name]
                RT = np.linalg.inv(self.extrinsics[camera_name])
            ...
            ply_dict_background_visible[frame_name] = visible_mask
        ...
        for track_id in os.listdir(lidar_actor_dir):
            ...
            for actor_ply_path in actor_ply_list:
                frame_name = file_path_to_frame_name(os.path.basename(actor_ply_path))
    ...
    def check_file_path(self, file_path):
        frame_name = file_path_to_frame_name(file_path)
        cam = image_filename_to_cam(file_path)
        frame = self.frame_name_list.index(frame_name)
        if cam in self.cams and frame >= self.start_frame and frame <= self.end_frame:
            return True
        else:
            return False
        
    def make_lidar_ply_st(self, start_frame, end_frame, actor_ids, frame_name_list):
        ply_frame_dict = dict()
        bkgd_ply = []
        for frame in range(start_frame, end_frame + 1):
            frame_name = frame_name_list[frame]
            bkgd_ply.append(self.ply_dict['background'][frame_name])
        ...
        for actor_id in actor_ids:
            ...
            for frame in range(start_frame, end_frame + 1):
                frame_name = frame_name_list[frame]
                if frame_name not in self.ply_dict[actor_id]:
                    continue
                actor_ply.append(self.ply_dict[actor_id][frame_name])
    ...
    def initailize_ply(self, datadir, objects_info):
        ...
        if os.path.exists(sky_mask_dir) and not os.path.exists(ply_sky_path):
            ...
            for i, sky_mask_path in enumerate(sky_mask_paths):
                ...
                camera_name = self.camera_id_names_dict[cam]
                K = self.intrinsics[camera_name]
                c2w = self.ego_frame_poses[frame] @ self.extrinsics[camera_name]
    ...
    def render_condition(self, camera: Camera, objects_info):
        ...
        frame_name = self.frame_name_list[frame]
        ...
        for actor in objects_info.values():
            if frame >= actor['start_frame'] and frame <= actor['end_frame']:
                actor_ids.append(str(actor['track_id']))

        ply_frame_dict = self.make_lidar_ply_st(start_frame=start_frame, end_frame=end_frame, actor_ids=actor_ids, frame_name_list=self.frame_name_list )
        ...
        track_info_frame = self.track_info[frame_name]
