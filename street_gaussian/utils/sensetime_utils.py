...
from data_processor.sensetime_processor.sensetime_helpers import image_heights, image_widths, image_filename_to_frame, image_filename_to_cam, camera_names_dict
from data_processor.sensetime_processor.sensetime_helpers import  waymo_track2label, file_path_to_frame_name, load_track, load_sensor_sync, load_calibration, load_centered_vehicle_poses
...
def get_obj_pose_tracking(datadir, selected_frames, cameras):
    ...
    for frame in range(start_frame, end_frame + 1):
        ...
        for cam_name in cameras:
            visible_id = frame_camera_visible[cam_name]
    ...
    unique_track_ids = [track_id for track_id in unique_track_ids if not trajectory[track_id]['stationary']] # modified
    ...
    for i, track_id in enumerate(unique_track_ids):
        ...
        frames = [int(frame_name) for frame_name in trajectory[track_id]['frames']]
...
def generate_dataparser_outputs(
    ...
    camera_id_names_dict = dict()
    for camera_name in camera_names_dict:
        camera_id_names_dict[int(camera_names_dict[camera_name])] = camera_name
    camera_names = [camera_id_names_dict[c_id] for c_id in cameras]
    ...
    sync_sensors = load_sensor_sync(datadir)
    num_frames_all = len(sync_sensors)
    ...
    ego_frame_poses = load_centered_vehicle_poses(datadir)
    frame_poses = [ego_frame_poses[ind] for ind in ego_frame_poses]

    # load camera, frame, path
    frames, frames_idx, cams, image_filenames = [], [], [], []
    ixts, exts, poses, c2ws, select_frame_poses = [], [], [], [], []
    cams_timestamps = []
    tracklet_timestamps = []
    ...
    for frame in range(start_frame, end_frame + 1):
        tracklet_timestamps.append(timestamps['FRAME'][f'{frame:06d}'])  # assume the tracklet timestamp is the same as the front camera

    for image_filename in image_filenames_all:
        image_basename = os.path.basename(image_filename)
        cam = image_filename_to_cam(image_basename)
        frame_name = file_path_to_frame_name(image_basename)
        ...
        if frame >= start_frame and frame <= end_frame and cam in cameras:
            ...
            pose = ego_frame_poses[frame_name]
            c2w = pose @ ext
            ...
            poses.append(pose)
            c2ws.append(c2w)
            select_frame_poses.append(ego_frame_poses[frame_name])

            timestamp = timestamps[camera_name][frame_name+'_'+camera_names_dict[camera_name]]
    ...
    select_frame_poses = np.stack(select_frame_poses, axis=0)
    ...
    result['ixts'] = ixts
    result['c2ws'] = c2ws
    result['ego_cam_poses'] = select_frame_poses
    result['ego_frame_poses'] = frame_poses
